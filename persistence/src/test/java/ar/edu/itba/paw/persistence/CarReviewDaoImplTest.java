...
public class CarReviewDaoImplTest {
    ...
    private static final Car CAR_2 = new Car(CAR_ID_2,"BB000BB","Fit Azul",USER_1, KNOWN_IMAGE_ID);
    ...
    public void testCreateCarReview(){
        ...
        //Assert
        TypedQuery<CarReview> query = em.createQuery("from CarReview r where r.id = :reviewId",CarReview.class);
        query.setParameter("reviewId",ans.getReviewId());
        Optional<CarReview> res = query.getResultList().stream().findFirst();
    ...
    @Test
    public void testGetCarReviewsEmpty(){
        //Execute
        PagedContent<CarReview> ans = carReviewDao.getCarReviews(CAR_2,0,PAGE_SIZE);
        //Assert
    ...
    public void testFindByIdPresent(){
        //Execute
        Optional<CarReview> ans = carReviewDao.findById(REVIEW_ID);
        //Assert
    ...
    @Test
    public void testGetCarReviewsMadeByUserOnTrip(){
        //Setup
        Trip auxTrip = em.merge(TRIP_2);
        User auxUser = em.merge(USER_2);
        CAR_1.setBrand(CarBrand.UNKNOWN);
        Car auxCar = em.merge(CAR_1);
        //Execute
        PagedContent<CarReview> ans = carReviewDao.getCarReviewsMadeByUserOnTrip(auxCar,auxUser,auxTrip,0,PAGE_SIZE);
        //Assert
        Assert.assertEquals(1,ans.getTotalCount());
        Assert.assertEquals(3,ans.getElements().get(0).getReviewId().longValue());
    }
