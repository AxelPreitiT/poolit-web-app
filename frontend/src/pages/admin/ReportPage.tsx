...
const ReportPage = () => {
    ...
    const { isLoading: isReportLoading,  report : report } = useReportById(params.id);
    const [UserReporter, setUserReporter] = useState<UserPublicModel | null>(null);
    const [UserReported, setUserReported] = useState<UserPublicModel | null>(null);
    const {isLoading: isTripLoading, trip, isError: isTripError} = useTripByUri(report?.tripUri);
    ...
    <MainComponent>
      <MainHeader title={t("admin.report.detail")} />
        <div className={styles.container_tab}>
          <div>
            {report == undefined || UserReporter == undefined || UserReported == undefined || isReportLoading ? (
                <SpinnerComponent />
            ) : (
                <div  className={styles.report_content_container2}>
                    <div className={styles.report_users_content_container}>

                        <div id="users-image-container"  className={styles.users_container}>
                            ...
                            <div  className={styles.report_arrow_container}>
                                <div className={styles.report_arrow_content}>
                                    ...
                                    <div className={styles.report_arrow_draw} id="report-arrow-draw">
                                        <div className={styles.report_arrow_line} id="report-arrow-line"></div>
                                        <div className={styles.report_arrow_head} id="report-arrow-head"></div>
                                    </div>
                                </div>
                            </div>
                            <div className={styles.user_container}>
                        ...
                        <div id="users-container" className={styles.users_container}>
                            <div className={styles.user_container}>
                                ...
                                {report.relation == "PASSENGER_2_DRIVER" || report.relation == "PASSENGER_2_PASSENGER" ?
                                    (
                                        <StarRating rating={UserReporter.passengerRating} size="x-large" />
                                    ) : (
                                        <StarRating rating={UserReporter.driverRating} size="x-large" />
                                    )
                                }
                                <div  className={styles.user_container_item + styles.user_container_row}>
                                    <strong>
                            ...
                            <div className={styles.user_container}>
                                ...
                                {report.relation == "DriverToPassanger" || report.relation == "PassangerToPassanger" ?
                                    (
                                        <StarRating rating={UserReported.passengerRating} size="x-large" />
                                    ) : (
                                        <StarRating rating={UserReported.driverRating} size="x-large" />
                                    )
                                }
                                <div className={styles.user_container_item + styles.user_container_row}>
                                    <strong>
