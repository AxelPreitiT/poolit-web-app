...
describe("Home",()=>{
    ...
    it("Should not show the tutorial page when there are not recommended trips",async ()=>{
        setAuthToken();
        server.use(UserMock.getByIdPrivateRoleDriver(),CityMock.getAllCities(),CarFeatureMock.getAllFeatures(),TripMock.mockEmptyTrips())
    ...
    it("Should show the tutorial when there are recommended trips",async ()=>{
        setAuthToken();
        server.use(UserMock.getByIdPrivateRoleDriver(),CityMock.getAllCities(),CarFeatureMock.getAllFeatures(),TripMock.mockTripList())

        customRender(<HomePage/>, {route:"/"})

        expect(screen.queryByText(/Find your trip in CABA/i)).not.toBeInTheDocument();
