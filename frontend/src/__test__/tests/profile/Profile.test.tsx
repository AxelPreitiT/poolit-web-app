...
import UserMock, {profileListInfo, publicUser} from "@/__test__/mocks/UserMock.ts";
...
describe("Profile", () => {
  ...
  it("Should show the public profile info", async () => {
    customRender(<PublicProfileInfo user={publicUser} />, { route: "/profile/5" });
    expect(await screen.findByText(publicUser.username + " " + publicUser.surname)).toBeVisible();
  ...
  it("Should show the empty format", async () => {
    customRender(
        ...
        link={publicsDriverReviewsPath.replace(":id", String(profileListInfo.id))}
    />)
    expect(await screen.findByText(profileListInfo.empty_text)).toBeVisible();
    expect(await screen.queryByText(profileListInfo.btn_footer_text)).not.toBeInTheDocument();
  });
  ...
  it("Should show the fill format", async () => {
    const {user } = customRender(
        <ListProfileContainer
            ...
            data={reviewList}
            component_name={ShortReviewProfile}
            link={publicsPassangerReviewsPath.replace(":id", profileListInfo.id.toString())}
        />)
    const listButton = screen.getByRole("link", { name:  profileListInfo.btn_footer_text});

    expect(await screen.queryByText(profileListInfo.empty_text)).not.toBeInTheDocument();
    expect(await screen.findByText(profileListInfo.btn_footer_text)).toBeVisible();

    await user.click(listButton);

    expect(window.location.pathname).toBe(publicsPassangerReviewsPath.replace(":id", profileListInfo.id.toString()));
